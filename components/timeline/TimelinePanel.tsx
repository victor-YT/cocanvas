    ...
    <div className="flex h-full flex-col">
      <div className="border-b border-zinc-200 px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold">Graph Event Timeline</h2>
            <p className="text-xs text-zinc-500">{eventCount} graph events</p>
          </div>
          {isReplaying ? (
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3">
        {events.length === 0 ? (
          <div className="grid min-h-[220px] place-items-center rounded-lg border border-dashed border-zinc-300 bg-zinc-50 p-6 text-center">
            <div>
              <p className="text-sm font-medium text-zinc-800">No graph events yet</p>
              <p className="mt-1 text-xs text-zinc-500">
                Run the demo to append JSONL-style graph events.
              </p>
            </div>
          </div>
        ) : (
